...
var import_utils = require("@unique-nft/utils");
...
var safelyDecodeUTF8String = (hex) => {
  try {
    return import_string.Utf8.hexStringToString(hex);
  } catch (e) {
    return null;
  }
};
var decodeHexAndParseJSONOrReturnNull = (hexString) => {
  try {
    return JSON.parse(import_utils.StringUtils.Utf8.hexStringToString(hexString));
  } catch {
    return null;
  }
};
...
var hexifyProperties = (properties) => {
  return properties.map((p) => ({
    key: p.key,
    valueHex: p.valueHex ?? import_string.Utf8.stringToHexString(p.value)
  }));
};
...
var import_royalties2 = require("@unique-nft/utils/royalties");
...
var decodeOldSchemaCollection = (properties, decodingImageLinkOptions) => {
  ...
  const propObj = properties.reduce((acc, { key, value, valueHex }) => {
    acc[key] = value ?? import_utils2.StringUtils.Utf8.hexStringToString(valueHex);
    return acc;
  }, {});
...
var decodeOldSchemaToken = (propertiesArray, schema, decodingImageLinkOptions, options) => {
  ...
  const u8aToken = import_utils2.StringUtils.HexString.toU8a(constDataProp.valueHex);
  let tokenDecoded = {};
...
var import_utils4 = require("@unique-nft/utils");
...
var fillTokenFieldByKeyPrefix = (token, properties, prefix, tokenField) => {
  ...
  if (urlInfixProperty)
    field.urlInfix = import_utils4.StringUtils.Utf8.hexStringToString(urlInfixProperty.valueHex);
  const urlProperty = properties.find(({ key }) => key === keysMatchingPrefix[1]);
  if (urlProperty)
    field.url = import_utils4.StringUtils.Utf8.hexStringToString(urlProperty.valueHex);
  const ipfsCidProperty = properties.find(({ key }) => key === keysMatchingPrefix[2]);
  if (ipfsCidProperty)
    field.ipfsCid = import_utils4.StringUtils.Utf8.hexStringToString(ipfsCidProperty.valueHex);
  const hashProperty = properties.find(({ key }) => key === keysMatchingPrefix[3]);
  if (hashProperty)
    field.hash = import_utils4.StringUtils.Utf8.hexStringToString(hashProperty.valueHex);
};
...
var import_utils6 = require("@unique-nft/utils");
var import_royalties = require("@unique-nft/utils/royalties");
...
var decodeV0OrV1CollectionSchemaToIntermediate = (properties, schemaFamily, options) => {
  ...
  const royalties = royaltyEncoded ? import_royalties.Royalties.uniqueV2.decode(royaltyEncoded) : [];
  if (royalties.length)
...
var decodeV0OrV1TokenToIntermediate = (propertyArray, schema, options, imageLinkOptions) => {
  ...
  if (owner && import_utils6.Address.is.nestingAddress(owner)) {
    decodedToken.nestingParentToken = import_utils6.Address.nesting.addressToIds(owner);
  }
  return decodedToken;
};
...
var decodeTokenUniqueV0OrV1 = async (tokenProperties, schemaFamily, options) => {
  ...
  const tokenRoyalties = tokenRoyaltyEncoded ? import_royalties2.Royalties.uniqueV2.decode(tokenRoyaltyEncoded) : [];
  const collectionRoyaltiesEncoded = collectionProperties?.find((prop) => prop.key === "royalties")?.valueHex;
  const collectionRoyalties = collectionSchema.royalties ? collectionSchema.royalties : collectionRoyaltiesEncoded ? import_royalties2.Royalties.uniqueV2.decode(collectionRoyaltiesEncoded) : [];
...
// src/encoding.ts
var import_royalties3 = require("@unique-nft/utils/royalties");
...
var encodeToken = (data, options) => {
  ...
  token.royalties && properties.push({ key: "royalties", valueHex: import_royalties3.Royalties.uniqueV2.encode(token.royalties) });
